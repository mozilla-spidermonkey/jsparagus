...
def stack_value(ast):
    ...
    with open("../generated_parser/src/stack_value_generated.rs", "w+") as f:
        def write(*args):
            write_impl(f, *args)
        write(0, "// WARNING: This file is auto-generated by crates/ast/generate_ast.py.")
        write(0, "")
...
def dump(ast):
    with open('src/dump_generated.rs', 'w+') as f:
        ...
        write(0, 'use std::io;')
        write(0, '')
        write(0, 'fn newline<W>(out: &mut W, depth: usize)')
        write(1, 'where W: io::Write')
        write(0, '{')
        write(1, 'writeln!(out, "").expect("failed to dump");')
        write(1, 'for i in 0..depth {')
        ...
        write(1, '{')
        write(2, 'if *self {')
        write(3, 'write!(out, "true").expect("failed to dump");')
        write(2, '} else {')
        write(3, 'write!(out, "false").expect("failed to dump");')
        write(2, '}')
        write(1, '}')
        write(0, '}')
        write(0, '')
